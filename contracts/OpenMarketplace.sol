...
contract OpenALCHIMarketplace is ERC1155Holder, ReentrancyGuard, Ownable {
    ...
    constructor(address _alchemyGame) Ownable(msg.sender) {
    ...
    function listItem(uint256 tokenId, uint256 amount, uint256 price, uint256 swapTokenId) external {
        ...
        if (swapTokenId != 0) {
            ...
            require(listedRarity == swapRarity, "Swap token must have same rarity");
        }
        
        alchemyGame.safeTransferFrom(msg.sender, address(this), tokenId, amount, "");
        
        listingCounter++;
        listings[listingCounter] = Listing(msg.sender, tokenId, amount, price, swapTokenId);
